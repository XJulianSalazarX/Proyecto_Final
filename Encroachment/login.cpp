...
void login::on_next_clicked()
{
    ...
    if (ui->password_2->isVisible()) {
        ...
        else if (login::existUser(user)==-1) {
            login::adduser(user,password);
...
string password_=password.toStdString();
 ...
 text=LeerArchivo();
 text=login::Str_to_Binary(text);
 text=login::decod( text);
 text=login::Binary_to_Str( text);
